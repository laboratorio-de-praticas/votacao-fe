...
const Header = ({ children }) => (
  <header className="mb-4 md:mb-6 lg:mb-8">
    <h1 className="text-[#004854] text-xl md:text-3xl lg:text-[45px] leading-tight">
      ESCOLHA DE <span className="font-bold block">REPRESENTANTES</span>
    </h1>
...
const CandidateInfoRow = ({ label, value, className = "" }) => (
  <p className={`text-[#1A6C7C] md:text-[#4A4A4A] ${className}`}>
    <span className="text-[#004854] hidden md:inline font-bold text-[25px] mr-2">
      {label}:
    </span>
    {value}
  </p>
...
const CandidateCard = ({ image, mobileImage, name, email, room }) => (
  <article className="w-full max-w-[171px] md:max-w-[386px] lg:max-h-[406px] xl:max-h-none bg-white rounded-b-[14px] shadow-lg hover:shadow-xl transition-shadow">
    <div className="relative aspect-square md:aspect-[386/242]">
      {/* Mobile Image */}
    ...
    <div className="p-4 md:p-6 space-y-2 md:space-y-3">
      <CandidateInfoRow
        label="Nome"
        value={name}
        className="font-bold md:font-normal text-[15px] md:text-[23px]"
      />
      <CandidateInfoRow
        label="Email"
        value={email}
        className="text-[12px] md:text-[23px]"
      />
      <CandidateInfoRow
        label="Sala"
        value={room}
...
const ActionButton = ({ children, onClick, className = "" }) => (
  <button
    onClick={onClick}
    className={`px-6 py-2 bg-[#9D0000] hover:bg-[#a31515] text-white rounded-full 
      ...
      <Header>
        ...
        <div className="md:hidden flex flex-col items-center">
          <div className="w-full">
            ...
            <p className="text-sm mb-4 text-justify">
              Parabéns! Seu voto está quase confirmado, verifique se o candidato é o desejado e
              confirme logo abaixo. Em breve atualizaremos você sobre o resultado da votação e
              quem serão os novos representantes de sua turma!
            </p>
          </div>
        ...
        <div className="hidden md:grid md:grid-cols-2 md:gap-8 lg:gap-12">
          ...
          <div className="flex flex-col justify-around">
            <h2 className="text-[#004854] font-bold text-3xl lg:text-[45px] mb-4">
              VOTO QUASE CONFIRMADO!
            </h2>
